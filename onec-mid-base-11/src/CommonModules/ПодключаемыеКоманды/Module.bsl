...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	Если ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" Тогда
		...
		ДобавитьМеханизмСогласованияСкидкиИПересчетаТаблиц(Форма);		
...
Процедура ВставитьПолеКонтактноеЛицоПередСуммаДокумента(Форма)
	...
	КонтактноеЛицо = Форма.Элементы.Вставить("КонтактноеЛицо", Тип("ПолеФормы"),, Форма.Элементы.СуммаДокумента);
	КонтактноеЛицо.Заголовок = "Контактное лицо";
	КонтактноеЛицо.Вид = ВидПоляФормы.ПолеВвода;
	КонтактноеЛицо.ПутьКДанным = "Объект.Net_КонтактноеЛицо";
...
Процедура ВставитьПолеКонтактноеЛицоПередОснование(Форма)
	...
	КонтактноеЛицо = Форма.Элементы.Вставить("КонтактноеЛицо", Тип("ПолеФормы"),, Форма.Элементы.Основание);
	КонтактноеЛицо.Заголовок = "Контактное лицо";
	КонтактноеЛицо.Вид = ВидПоляФормы.ПолеВвода;
	КонтактноеЛицо.ПутьКДанным = "Объект.Net_КонтактноеЛицо";
...
Процедура ДобавитьПолеКонтактноеЛицоВГруппаШапкаПраво(Форма)
	...
	КонтактноеЛицо = Форма.Элементы.Добавить("КонтактноеЛицо", Тип("ПолеФормы"), Форма.Элементы.ГруппаШапкаПраво);
	КонтактноеЛицо.Заголовок = "Контактное лицо";
	КонтактноеЛицо.Вид = ВидПоляФормы.ПолеВвода;
	КонтактноеЛицо.ПутьКДанным = "Объект.Net_КонтактноеЛицо";
...
Процедура ДобавитьМеханизмСогласованияСкидкиИПересчетаТаблиц(Форма)
	...
	КомандаПересчитать.Картинка = БиблиотекаКартинок.Перечитать;
	
	// Добавление группы ГруппаСогласованнаяСкидка
	...
	// Добавление поля СогласованнаяСкидка
	СогласованнаяСкидка = Форма.Элементы.Добавить("СогласованнаяСкидка", Тип("ПолеФормы"), Форма.Элементы.ГруппаСогласованнаяСкидка);
	СогласованнаяСкидка.Заголовок = "Согласованная скидка";
	СогласованнаяСкидка.Вид = ВидПоляФормы.ПолеВвода;
	СогласованнаяСкидка.ПутьКДанным = "Объект.Net_СогласованнаяСкидка";
	СогласованнаяСкидка.УстановитьДействие("ПриИзменении", "Подключаемый_СогласованнаяСкидкаПриИзменении");
	
	// Добавление кнопки пересчета таблиц с учетом скидки
	КнопкаПересчитать = Форма.Элементы.Добавить("ПересчитатьТаблицу", Тип("КнопкаФормы"), Форма.Элементы.ГруппаСогласованнаяСкидка);
